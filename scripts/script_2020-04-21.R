...
p <- 
  ...
  scale_x_upset(order_by = "freq", n_intersections = counts$unique_composition[2]) +
  labs(x = NULL, y = NULL, 
       title = "Count of GDPR articles violations compositions",
       subtitle = glue("Compositions that happened {limit_chr} times or less are not displayed \\
                       ({counts$total_by_composition[1]} fines and \\
                       {length(setdiff(counts$article[[1]], counts$article[[2]]))} \\
                       other articles)."),
       caption = "Source: Privacy Affairs\n@_abichat for #TidyTuesday") +
  theme_minimal() +
  theme(title = element_text(color = "#C7AB47", size = 20, family = family), 
        plot.subtitle = element_text(size = 13),
        plot.caption = element_text(size = 11),
        plot.title.position = "plot",
        axis.text.y = element_text(hjust = 0, color = "#C7AB47", size = 10, family = family),
  ...
  theme_combmatrix(combmatrix.panel.point.color.fill = "black", 
                   ...
                   combmatrix.label.height = unit(10, "cm"))
